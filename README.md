...
- **Live resize mode** — drag the corner handle to make the launcher any size you like, then click **Apply**. A **Reset** button restores the default `400 × 250` size.
- **Auto-hide on focus loss** — disappears when you click away (configurable behaviour for drag mode and dialogs).
- **Tray context menu** with **Exit DockTray** action.
...
| **Acrylic** | True Windows **acrylic blur** behind the launcher, with a **whiteness slider (0–100 %)**. Drag it left for a dark, glassy look; drag it right for a frosted / milky look. Includes an **Apply** and **Reset** button. |
...
- **Drag & Drop mode** (↔️) — toggle Drag Mode, then **drop files / folders / apps directly onto the launcher** to add them.

### ⚙️ Settings
A polished **FluentWindow** (qfluentwidgets) with four sub-pages, accessible from the **Settings** (⚙️) button:
...
3. **Add shortcuts** — use the bottom bar buttons (📄 Add File, 📁 Add Folder, 🟦 Add Windows Apps), or enable Drag Mode (↔️) and drop items onto the launcher.
...
6. **Customize** — click the **Settings** (⚙️) button in the bottom bar to open the settings window.
7. **Resize the launcher** — open Settings → **Positioning** → **Resize Tray**, then drag the ◢ handle in the top-left of the launcher to the size you want, then **Apply** (or **Cancel** to revert).
...
│   │   └── icon_extractor.py     # HICON → QPixmap, UWP manifest logo resolution, AppsFolder PIDL
│   ├── ui/
...
│   │   └── ui_utils.py           # SVG → QIcon, built-in icon set
...
| `theme_whiteness` | `0` | `0` (dark acrylic) → `100` (pure white acrylic). |
...
| `shortcut_icon_size` | `45` | Shortcut tile icon size in pixels (28–72). |
| `launcher_button_size` | `40` | Bottom-bar button size in pixels (32–56). |
